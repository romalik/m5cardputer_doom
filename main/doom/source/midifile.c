...
static boolean ReadTrack(midi_track_t *track, midimem_t *mf)
{
    ...
    // instead of allocating mem for events, pin pointer to start of events, parse them on the go
    track->evt_base = &(mf->data[mf->pos]);
...
midi_file_t *MIDI_LoadFile (midimem_t *mf)
{
    ...
    if (file == NULL)
    {
        printf("malloc(sizeof(midi_file_t)) failed\n");
        return NULL;
    }
